...
class SoccerPitch:
    ...
    def create_lines(self, points_3d):
        ...
        R = 9.15
        c = points_3d[25]
        lines.append(c[None, :] + R * np.stack([np.cos(ARC), np.sin(ARC), np.zeros_like(ARC)]).T)
        ...
        c = points_3d[22]
        lines.append(c[None, :] + R * np.stack([np.cos(CIRCLE), np.sin(CIRCLE), np.zeros_like(CIRCLE)]).T)
        return lines
