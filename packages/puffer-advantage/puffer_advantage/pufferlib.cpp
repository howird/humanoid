...
void vtrace(float *values, float *rewards, float *dones, float *importance,
            float *trace, float gamma, float rho_clip, float c_clip,
            int num_steps, const int horizon) {
  for (int offset = 0; offset < num_steps * horizon; offset += horizon) {
    vtrace_row(values + offset, rewards + offset, dones + offset,
               importance + offset, trace + offset, gamma, rho_clip, c_clip,
               horizon);
  }
}
...
void puff_advantage(float *values, float *rewards, float *dones,
                    float *importance, float *trace, float gamma,
                    float rho_clip, float c_clip, int num_steps,
                    const int horizon) {
  for (int offset = 0; offset < num_steps * horizon; offset += horizon) {
...
torch::Tensor compute_vtrace(torch::Tensor values, torch::Tensor rewards,
  ...
  int horizon = values.size(1);
  torch::Tensor trace =
      vtrace_check(values, rewards, dones, importance, num_steps, horizon);
...
torch::Tensor compute_puff_advantage(torch::Tensor values,
                                     torch::Tensor rewards, torch::Tensor dones,
                                     torch::Tensor importance, float gamma,
