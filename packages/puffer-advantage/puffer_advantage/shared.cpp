...
// [horizon]
__host__ __device__ void gae_row(float *values, float *rewards, float *dones,
                                 float *advantages, float gamma,
                                 float gae_lambda, int horizon) {
  float lastgaelam = 0;
  for (int t = horizon - 2; t >= 0; t--) {
    ...
    float delta =
        rewards[t_next] + gamma * values[t_next] * nextnonterminal - values[t];
...
__host__ __device__ void vtrace_row(float *values, float *rewards, float *dones,
  ...
  for (int t = horizon - 2; t >= 0; t--) {
    ...
    float delta =
        rho_t *
        (rewards[t] + gamma * values[t_next] * nextnonterminal - values[t]);
    accum = delta + gamma * c_t * accum * nextnonterminal;
    advantages[t] =
        rho_t * (rewards[t] + gamma * vs[t_next] * nextnonterminal - values[t]);
    vs[t] = accum + values[t];
  }
}

__host__ __device__ void
puff_advantage_row(float *values, float *rewards, float *dones,
                   float *importance, float *vs, float *advantages, float gamma,
  ...
  for (int t = horizon - 2; t >= 0; t--) {
    ...
    // gamma*lambda*lastpufferlam*nextnonterminal;

    advantages[t] = lastpufferlam;
    vs[t] = advantages[t] + values[t];
    // advantages[t] = rho_t*(rewards[t] + gamma*vs[t_next]*nextnonterminal -
    // values[t]); vs[t] = lastpufferlam + values[t];
  }
}

void vtrace_check(torch::Tensor values, torch::Tensor rewards,
                  torch::Tensor dones, torch::Tensor importance,
                  torch::Tensor vs, torch::Tensor advantages, int num_steps,
                  int horizon) {

  // Validate input tensors
  torch::Device device = values.device();
  for (const torch::Tensor &t :
       {values, rewards, dones, importance, vs, advantages}) {
    TORCH_CHECK(t.dim() == 2, "Tensor must be 2D");
    TORCH_CHECK(t.device() == device, "All tensors must be on same device");
    TORCH_CHECK(t.size(0) == num_steps, "First dimension must match num_steps");
    TORCH_CHECK(t.size(1) == horizon, "Second dimension must match horizon");
