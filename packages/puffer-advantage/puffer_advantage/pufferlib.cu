...
__global__ void p3o_kernel(float *reward_block, // [num_steps, horizon]
                           ...
                           int num_steps, float r_std, float puf, int horizon) {
  ...
  // if (k == horizon-1) {
  //     bootstrap = buf[i*horizon + horizon - 1]*values_mean[i*horizon +
  //     horizon - 1];
  // }
...
void compute_p3o(torch::Tensor reward_block, torch::Tensor reward_mask,
                 torch::Tensor values_mean, torch::Tensor values_std,
                 torch::Tensor buf, torch::Tensor dones, torch::Tensor rewards,
                 torch::Tensor advantages, torch::Tensor bounds, int num_steps,
                 float vstd_max, float puf, int horizon) {

  // TODO: Port from python
  /*
  assert all(t.is_cuda for t in [reward_block, reward_mask, values_mean,
  values_std, buf, dones, rewards, advantages, bounds]), "All tensors must be on
  GPU"

  # Ensure contiguous memory
  tensors = [reward_block, reward_mask, values_mean, values_std, buf, dones,
  rewards, advantages, bounds] for t in tensors: t.contiguous() assert t.is_cuda
  ...
  */
  ...
  p3o_kernel<<<blocks, threads_per_block>>>(
      reward_block.data_ptr<float>(), reward_mask.data_ptr<float>(),
      values_mean.data_ptr<float>(), values_std.data_ptr<float>(),
      buf.data_ptr<float>(), dones.data_ptr<float>(), rewards.data_ptr<float>(),
      advantages.data_ptr<float>(), bounds.data_ptr<int>(), num_steps, vstd_max,
      puf, horizon);
...
__global__ void gae_kernel(float *values, float *rewards, float *dones,
                           float *advantages, float gamma, float gae_lambda,
...
torch::Tensor compute_gae(torch::Tensor values, torch::Tensor rewards,
  ...
  torch::Tensor advantages =
      gae_check(values, rewards, dones, num_steps, horizon);
  ...
  gae_kernel<<<blocks, threads_per_block>>>(
      values.data_ptr<float>(), rewards.data_ptr<float>(),
      dones.data_ptr<float>(), advantages.data_ptr<float>(), gamma, gae_lambda,
      num_steps, horizon);
...
// [num_steps, horizon]
__global__ void vtrace_kernel(float *values, float *rewards, float *dones,
                              float *importance, float *vs, float *advantages,
                              float gamma, float rho_clip, float c_clip,
                              int num_steps, int horizon) {
  int row = blockIdx.x * blockDim.x + threadIdx.x;
  int offset = row * horizon;
  vtrace_row(values + offset, rewards + offset, dones + offset,
             importance + offset, vs + offset, advantages + offset, gamma,
...
void compute_vtrace(torch::Tensor values, torch::Tensor rewards,
                    ...
                    torch::Tensor vs, torch::Tensor advantages, float gamma,
                    float rho_clip, float c_clip) {
  int num_steps = values.size(0);
  int horizon = values.size(1);
  vtrace_check(values, rewards, dones, importance, vs, advantages, num_steps,
               horizon);
  TORCH_CHECK(values.is_cuda(), "All tensors must be on GPU");
  ...
  vtrace_kernel<<<blocks, threads_per_block>>>(
      ...
      vs.data_ptr<float>(), advantages.data_ptr<float>(), gamma, rho_clip,
      c_clip, num_steps, horizon);
...
__global__ void puff_advantage_kernel(float *values, float *rewards,
                                      float *dones, float *importance,
                                      float *vs, float *advantages, float gamma,
                                      float lambda, float rho_clip,
                                      float c_clip, int num_steps,
                                      int horizon) {
  int row = blockIdx.x * blockDim.x + threadIdx.x;
  int offset = row * horizon;
  puff_advantage_row(values + offset, rewards + offset, dones + offset,
                     importance + offset, vs + offset, advantages + offset,
                     gamma, lambda, rho_clip, c_clip, horizon);
}

void compute_puff_advantage(torch::Tensor values, torch::Tensor rewards,
                            torch::Tensor dones, torch::Tensor importance,
                            torch::Tensor vs, torch::Tensor advantages,
                            float gamma, float lambda, float rho_clip,
                            float c_clip) {
  int num_steps = values.size(0);
  int horizon = values.size(1);
  vtrace_check(values, rewards, dones, importance, vs, advantages, num_steps,
  ...
  puff_advantage_kernel<<<blocks, threads_per_block>>>(
      values.data_ptr<float>(), rewards.data_ptr<float>(),
      dones.data_ptr<float>(), importance.data_ptr<float>(),
      vs.data_ptr<float>(), advantages.data_ptr<float>(), gamma, lambda,
      rho_clip, c_clip, num_steps, horizon);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("compute_puff_advantage", &compute_puff_advantage,
        "Compute PuffAdvantage with CUDA");
}
