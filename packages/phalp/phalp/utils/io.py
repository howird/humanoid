    ...
    def get_frames_from_source(self):
        ...
        if source_path.startswith("https://") or source_path.startswith("http://"):
            ...
            youtube_video.streams.get_highest_resolution().download(
                output_path=self.cfg.video.output_dir + "/_DEMO/" + video_name, filename="youtube.mp4"
            )
        ...
        if source_path.endswith(".mp4"):
            ...
            if self.cfg.video.extract_video:
                ...
                fe.extract_frames(
                    every_x_frame=1,
                    img_name="",
                    dest_path=self.cfg.video.output_dir + "/_DEMO/" + video_name + "/img/",
                    start_frame=self.cfg.video.start_frame,
                    end_frame=self.cfg.video.end_frame,
                )
        ...
        if t == 0:
            self.video = {
                "video": cv2.VideoWriter(
                    video_path, cv2.VideoWriter_fourcc(*"mp4v"), self.output_fps, frameSize=f_size
                ),
                "path": video_path,
            }
