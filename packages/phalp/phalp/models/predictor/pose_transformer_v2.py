...
class lart_transformer(nn.Module):
    def __init__(
        ...
        self.smpl_head = nn.ModuleList(
            [
                SMPLHead(self.phalp_cfg, input_dim=self.cfg.in_feat, pool="pooled")
                for _ in range(self.cfg.num_smpl_heads)
            ]
        )

        # Location head for predicting 3D location of the person
        self.loca_head = nn.ModuleList(
            [
                nn.Sequential(
        ...
        ava_action_classes = (
            self.cfg.ava.num_action_classes
            if not (self.cfg.ava.predict_valid)
            else self.cfg.ava.num_valid_action_classes
        )
        self.action_head_ava = nn.ModuleList(
            [
                nn.Sequential(
...
class Pose_transformer_v2(nn.Module):
    ...
    def add_slowfast_features(self, fast_track):
        ...
        for t_, time_stamp in enumerate(list_iter):
            ...
            end_ = (
                (time_stamp + 1) * NUM_STEPS
                if (time_stamp + 1) * NUM_STEPS < len(list_of_frames)
                else len(list_of_frames)
            )
            time_stamp_ = list_of_frames[start_:end_]
            if len(time_stamp_) == 0:
                continue
    ...
    def smooth_tracks(self, fast_track, moving_window=False, step=1, window=20):
        ...
        if "mvit" in self.cfg.extra_feat.enable:
            mvit_feat_all = fast_track["mvit_emb"][
                None,
                :,
                :,
            ]
